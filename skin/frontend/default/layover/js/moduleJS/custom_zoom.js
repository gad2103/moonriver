		...
		this.fadedOut=function()
		{
				...
				this.controlLoop=function()
			{
				if(lens)
				{
					...
					if(lens){lens.hide(250)}
					if($padachhayo){$padachhayo.hide(250)}
					if(softFocus){softFocus.hide(250)}customZoomBlock.hide(251,function()
					{
						ctx.fadedOut()
					});
					return false
					...
					else
					{
						...
						customZoomBlock=appendTo.append(format('<div id="custom-zoom-big" class="custom-zoom-big" style="display:none;position:absolute;left:%0px;top:%1px;width:%2px;height:%3px;background-image:url(\'%4\');z-index:99;"></div>',
						xPos,
						yPos,
						w,
						h,
					...
					if($.browser.msie&&$.browser.version<7)
						{
							...
							zIndex:99,
							width:w,
							height:h}).insertBefore(customZoomBlock)}customZoomBlock.show(495);
					if(lens)
					{
						lens.remove();
						lens=null}
						cw=(smallImg.outerWidth()/zoomImage.width)*w;ch=(smallImg.outerHeight()/zoomImage.height)*h;
						lens=joomwindow.append(format("<div class = 'custom-zoom-lens' style='display:none;z-index:98;position:absolute;width:%0px;height:%1px;'></div>",cw,ch)).find(':last');
						$mouseTrap.css('cursor',lens.css('cursor'));
						var noTrans=false;
					if(opts.shade)
					{
						lens.css('background','url("'+smallImg.attr('src')+'")');
						$padachhayo=joomwindow.append(format('<div style="display:none;position:absolute; left:0px; top:0px; width:%0px; height:%1px; background-color:%2;" />',
						smallImg.outerWidth(),
						smallImg.outerHeight(),
						opts.shade)).find(':last');
						$padachhayo.css('opacity',opts.shadeOpacity);noTrans=true;
						$padachhayo.show(500)
					}
					if(opts.softFocus)
					{
						...
						noTrans=true;softFocus.show(495)
					}
					if(!noTrans)
					{
